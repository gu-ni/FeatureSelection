...
from swapping_autoencoder import SwappingAutoencoder
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        "--instance_data_dir",
        type=str,
        default='/mnt/Face_Private-NFS2/geonhui/FeatureSelection/diffusers/examples/dreambooth/image_folder/blue_illu_folder',
    ...
    parser.add_argument(
        "--instance_prompt",
        type=str,
        default='a woman working on laptop with blue plants and shelf on a white background in flat illustration style, minimal simple vector graphics',
        required=False,
    ...
    parser.add_argument(
        "--validation_prompt",
        type=str,
        default='a matt black sculpture of gorilla face',
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="lora-dreambooth-model_debugging_test",
...
class DreamBoothDataset(Dataset):
    ...
    def __init__(
        ...
        if args.dataset_name is not None or False:
            try:
...
def main(args):
    ...
    if args.train_swapping_autoencoder:
        text_encoder_one_dim = text_encoder_one.text_model.embeddings.token_embedding.embedding_dim
        text_encoder_two_dim = text_encoder_two.text_model.embeddings.token_embedding.embedding_dim
        in_features = text_encoder_one_dim + text_encoder_two_dim
        swapping_autoencoder = SwappingAutoencoder(in_features)
    ...
    swapping_autoencoder.to(accelerator.device, dtype=weight_dtype)
    ...
    if args.pretrained_lora_weight_dir != "":
    ...
    if not args.train_text_encoder and not train_dataset.custom_instance_prompts: ### guni: here
        ...
        ### guni
        if args.train_swapping_autoencoder:
            instance_prompt_hidden_states_upscaled, instance_pooled_prompt_embeds_upscaled = compute_text_embeddings(
                args.upscaled_prompt, text_encoders, tokenizers
            )
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            #### guni
            if args.train_swapping_autoencoder:
                assert args.swapping_autoencoder_weight > 0.0, "swapping_autoencoder_weight must be larger than 0.0"
                autoencoder_input = torch.cat([instance_prompt_hidden_states,
                                               instance_prompt_hidden_states_upscaled],
                                               dim=0)
                autoencoder_output = swapping_autoencoder(autoencoder_input)
                autoencoder_loss = F.mse_loss(autoencoder_input.float(), autoencoder_output.float(), reduction="mean")
            ...
            with accelerator.accumulate(unet):
                ...
                if args.train_swapping_autoencoder:
                    loss = loss + args.swapping_autoencoder_weight * autoencoder_loss
