...
def view_images(images: Union[np.ndarray, List],
    ...
    for i in range(num_rows):
        for j in range(num_cols):
            H = i * (h + offset)
            W = j * (w + offset)
            image_[H: H + h:, W: W + w] = images[i * num_cols + j]
...
def view_images_change(images: Union[np.ndarray, List],
                num_rows: int = 1,
                offset_ratio: float = 0.02,
                display_image: bool = True) -> Image.Image:
    """ Displays a list of images in a grid. """
    ...
    num_cols = num_items // num_rows
    image_ = np.ones((h * num_rows + offset * (num_rows - 1),
                      new_w * num_cols + offset * (num_cols - 1), 3), dtype=np.uint8) * 255
    for i in range(num_rows):
        for j in range(num_cols):
            start_col = j * (new_w + offset)
            end_col = start_col + new_w
            image_[i * (h + offset): i * (h + offset) + h, start_col:end_col] = images[i * num_cols + j]
...
class AttentionControl(abc.ABC):
    ...
    @abc.abstractmethod
    def forward (self, attn, is_self_attention: bool, place_in_unet: str):
        raise NotImplementedError

    @torch.no_grad()
    def __call__(self, attn, is_self_attention: bool, place_in_unet: str):
        if self.cur_att_layer >= self.num_uncond_att_layers:
            attn = self.forward(attn, is_self_attention, place_in_unet)
...
class AttentionStore(AttentionControl):
    ...
    def forward(self, attn, is_self_attention: bool, place_in_unet: str):
        ...
        key = f"{place_in_unet}_{'self' if is_self_attention else 'cross'}"
        if attn.shape[2] == 64 ** 2:  # 최소 32 이상이어야 함 # torch.Size([2, 10, 4096, 4096])
...
class AttentionControlEdit(AttentionStore, abc.ABC):
    ...
    def __init__(self, prompts, num_steps: int,
        ...
        self.local_blend = local_blend  # 외부에서 정의하고 전달
