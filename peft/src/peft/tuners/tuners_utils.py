...
class BaseTuner(nn.Module, ABC):
    ...
    def __init__(self, model, peft_config: Union[PeftConfig, dict[str, PeftConfig]], adapter_name: str,
        ...
        if not hasattr(self, "peft_config"):
            self.peft_config = {adapter_name: peft_config} if isinstance(peft_config, PeftConfig) else peft_config # {'dog': LoraConfig(peft_type...config={})}
        else:
            logger.info(
    ...
    def inject_adapter(self, model: nn.Module, adapter_name: str, feature_controller): # guni
        ...
        for key in key_list:
            ...
            if key.startswith("down_blocks"):
                ...
                place_in_unet = ""
                #raise Exception("key is not in down, mid, or up blocks")
            ...
            if 'attn1' in key:
                ...
                self_or_cross = ""
                #raise Exception("'attn1' or 'attn2' is not in key")
            
            ### cross 때 추가 ###
            attn_type = None
            attn_types = ['to_q', 'to_k', 'to_v', 'to_out']
            for temp in attn_types:
                if temp in key:
                    attn_type = temp
            if attn_type == None:
                attn_type = ""
                #raise Exception("'to_q', 'to_k', 'to_v', or 'to_out' is not in key")
            
            submodule_name = '.'.join(key.split('.')[:-1])
            submodule = model_modules[submodule_name]
            heads = submodule.heads if hasattr(submodule, 'heads') else None
            scale = submodule.scale if hasattr(submodule, 'scale') else 1.0
            
            optional_kwargs['place_in_unet'] = place_in_unet
            optional_kwargs['self_or_cross'] = self_or_cross
            optional_kwargs['attn_type'] = attn_type
            optional_kwargs['heads'] = heads
            ...
            # guni
            attn_layer_count += 1
        self.attn_layer_count = attn_layer_count
